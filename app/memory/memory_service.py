...
from app.config.settings import memory_config
...
def store_memory(task_obj: object, result: str, review: str, success: bool) -> None:
    ...
    doc = Document(
        ...
        metadata={
            "review": str(review), "success": str(success),
            "result": str(result)[:200], "timestamp": datetime.now().isoformat(),
            "type": "execution",
        },
    )
    try:
...
def retrieve_memory(query: object) -> str:
    ...
    try:
        ...
        query_engine = index.as_query_engine(similarity_top_k=memory_config.top_k)
